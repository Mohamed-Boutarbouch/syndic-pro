...
export const unitOwnerships = pgTable(
  ...
  (table) => [
    index("idx_unit_ownerships_unit_id").on(table.unitId),
    index("idx_unit_ownerships_user_id").on(table.userId),
    index("idx_unit_ownerships_active").on(table.unitId, table.startedAt),
  ],
);
